...
public class PlayerSkillController : MonoBehaviour
{
    ...
    void Update()
    {
        if (Input.GetKeyDown(KeyCode.X)) UseSkillByIndex(0);
        if (Input.GetKeyDown(KeyCode.C)) UseSkillByIndex(1);
        if (Input.GetKeyDown(KeyCode.V)) UseSkillByIndex(2);
        if (Input.GetKeyDown(KeyCode.B)) UseSkillByIndex(3);
    }
